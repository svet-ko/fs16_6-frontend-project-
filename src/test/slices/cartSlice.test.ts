...
describe('Testing action in cartReducer', () => {
    ...
    test("Should remove all products from the cart", () => {
      ...
      const newState = cartReducer(state, removeAllProductsFromCart(true));
      expect(newState.length).toBe(0);
    })

    test("Should not remove all products from the cart", () => {
      const state: CartItem[] = [
        {
        ...productsData[0],
        quantity: 2
      ...
      const newState = cartReducer(state, removeAllProductsFromCart(false));
